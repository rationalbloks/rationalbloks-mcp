...
'''
    ...
    async def generate_api_service(
        ...
        """Generate API service with CRUD operations."""
        project = Path(project_path).resolve()
...
'''
        ...
        for table_name in schema.keys():
            singular = self._get_singular(self._pascal_case(table_name))
            api_content += f"import type {{ {singular}, Create{singular}Input, Update{singular}Input }} from '../types/generated';\n"
...
'''
        ...
        return {
            "success": True,
            "file": str(api_file),
            "methods": [f"get{self._pascal_case(t)}, create{self._get_singular(self._pascal_case(t))}, update{self._get_singular(self._pascal_case(t))}, delete{self._get_singular(self._pascal_case(t))}" for t in schema.keys()],
        }
...
{initial_state}
  ...
  const loadData = async () => {{
    try {{
      setLoading(true);
      const data = await api.get{singular}(id!);
      setFormData(data);
      setError(null);
    }} catch (err) {{
...
'''
    ...
    async def get_template_structure(
        ...
        return {
            ...
            "key_dependency": "@rationalbloks/universalfront (auth, API client, utilities)",
        }
