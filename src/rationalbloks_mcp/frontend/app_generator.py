...
'''
    ...
    def _generate_api_service(self, project_path: Path, schema: dict, api_url: str | None) -> Path:
        # Generate API service using THE ONE WAY pattern
        # This uses createAuthApi + initApi + getApi from the npm packages.
        # All CRUD operations are handled generically by frontbuilderblok.
        
        # Build ENTITIES constant from schema
        entities_lines = []
        for table_name in schema.keys():
            entity_const = table_name.upper()
            entities_lines.append(f'  {entity_const}: "{table_name}"')
        entities_const = ",\n".join(entities_lines)
        
        api_content = f'''// ============================================================================
// AUTO-GENERATED API SERVICE - Do not edit manually
// Generated by RationalBloks Frontend MCP
// Uses THE ONE WAY pattern: createAuthApi + initApi + getApi
// ============================================================================
...
// Initialize auth API (provides token management)
const authApi = createAuthApi(API_URL);

// Initialize the generic API (uses authApi for auth headers)
initApi(authApi);

// Entity name constants for type-safe API calls
...
'''
    ...
    def _generate_list_view(self, project_path: Path, table_name: str, singular: str, fields: dict) -> Path:
        # Generate a list view component using THE ONE WAY pattern
        type_name = self._pascal_case(table_name)
        entity_const = table_name.upper()
...
// Uses THE ONE WAY pattern: getApi() from @rationalbloks/frontbuilderblok
// ============================================================================
...
'''
    ...
    def _generate_form_view(self, project_path: Path, table_name: str, singular: str, fields: dict) -> Path:
        # Generate a create/edit form view using THE ONE WAY pattern
        type_name = self._pascal_case(table_name)
        entity_const = table_name.upper()
...
'''
  ...
  async function loadData(itemId: string) {{
    try {{
      setLoading(true);
      const data = await getApi().getOne<{singular}>(ENTITIES.{entity_const}, itemId);
      setFormData({{
'''
...
'''
    ...
    def _generate_dashboard(self, project_path: Path, app_name: str, schema: dict) -> Path:
        # Generate a dashboard view using THE ONE WAY pattern
        view_content = f'''// ============================================================================
...
'''
        for table_name in schema.keys():
            entity_const = table_name.upper()
            view_content += f'        getApi().getAll(ENTITIES.{entity_const}),\n'
